        ...
        <div className={`note_view_container ${gridView ? 'grid_view' : ''}`}>
            ...
            {pinnedNotes?.length > 0 && pinnedNotes.map((element, index) => {
                return <div className='note_section' style={{ backgroundColor: colorList[element.color - 1].color }} key={element._id}>
                    ...
                    <input className='title_section title' style={{ backgroundColor: colorList[element.color - 1].color }} name="title" id="title" rows='1' placeholder='Title' value={element.title} readOnly onClick={() => {updateNote(element, element.color)}} />
                    <TextareaAutosize className='title_section note' style={{ backgroundColor: colorList[element.color - 1].color }} name="note" id="note" placeholder='Take a note...' autoFocus value={element.note} readOnly onClick={() => {updateNote(element, element.color)}} ></TextareaAutosize>
